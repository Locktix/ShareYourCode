(() => {
  ...
  const API_BASE = (typeof window.SYC_API_BASE === 'string') ? (window.SYC_API_BASE || '') : '';

  // When on index.html, we only need API_BASE for create/join handled inline.
  if (!isRoom) return;
  ...
  async function fetchRoom() {
    const res = await fetch(`${API_BASE}/api/room_state.php?roomId=${encodeURIComponent(roomId)}&action=get`);
  ...
  async function sendUpdate() {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/room_state.php`, {
        method: 'POST',
  ...
  async function sendUpdateMeta() {
    ...
    try {
      const payload = { action: 'update', roomId, code: codeMirror.getValue(), baseRevision: currentRevision, filename: filenameInput.value.trim() || 'snippet', language: languageSel.value };
      const res = await fetch(`${API_BASE}/api/room_state.php`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      const data = await res.json();
  ...
  async function fetchChat() {
    const res = await fetch(`${API_BASE}/api/chat.php?roomId=${encodeURIComponent(roomId)}&action=get`);
    const data = await res.json();
  ...
  // Bootstrap initial state then start polling
  initEditor();
  fetch(`${API_BASE}/api/room_state.php?roomId=${encodeURIComponent(roomId)}&action=get`).then(r => r.json()).then(d => {
    if (d.ok && d.room) {
      isApplyingRemote = true;
  ...
  function detectModeFromFilename(name) {
    ...
    if (lower.endsWith('.ts')) return 'javascript';
